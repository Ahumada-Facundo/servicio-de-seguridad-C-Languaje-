...
struct Trabajos_Pendientes {
    ...
    int idCliente;
    int idTecnico;
    int pendiente; // 1 si el trabajo est� pendiente, 0 si est� terminado
    char Ubicacion[50];
    struct fecha fech;
    float MonTotal; 
...
struct tareas *buscar_tareas_anterior(int opcion, int orden, int hora, int minutos, struct tareas *ini) {
    ...
    while (ini != NULL) {
        // Si la opci�n actual es mayor, hemos pasado el punto de inserci�n
        if (ini->cod_opcion > opcion) {
            break;
        }

        // Si encontramos la opci�n correcta
        if (ini->cod_opcion == opcion) {
            ...
            if (ini->orden == orden) {
                ...
                // Si encontramos una orden mayor, hemos pasado el punto de inserci�n
                break;
            }
        }
...
// Funci�n para crear y agregar un nuevo nodo de material
void nuevoNodoM(struct Material** raizM) {
...
// Funci�n para insertar un material en el �rbol
struct Material* insertarMaterial(struct Material* raizM, struct Material* nodoM) {
    if (raizM != NULL) {
...
// Funci�n para buscar el costo de un material por su ID
float busquedaMaterial(struct Material* raiz, int mat) {
    if (raiz != NULL) {
        if (raiz->CodMat == mat) {
        ...
        printf("No se encontr� material con ID %d.\n", mat);
        return -1;
    }
}

// Funci�n para recorrer y mostrar todos los materiales en el �rbol
void recorrer(struct Material* raiz) {
    if (raiz != NULL) {
...
// Funci�n para guardar los materiales en un archivo
void guardarMaterialesEnArchivo(struct Material* raiz, const char* nombreArchivo) {
    FILE* archivo = fopen(nombreArchivo, "w");
    if (archivo == NULL) {
        printf("Error al abrir el archivo");
...
// Funci�n auxiliar para guardar los materiales en un archivo
void guardarMaterialEnArchivo_auxiliar(FILE* archivo, struct Material* raiz) {
    if (raiz != NULL) {
...
// Funci�n para cargar los materiales desde un archivo
void cargarMaterialesDesdeArchivo(struct Material** raiz, const char* nombreArchivo) {
    FILE* archivo = fopen(nombreArchivo, "r");
    if (archivo == NULL) {
...
int convertirFechaADias(int dia, int mes, int anio) {
    // Simplificaci�n para convertir la fecha a un n�mero total de d�as
 ...
 void listarentrefechas_fina(struct Trabajos_Pendientes *E2, struct opciones *opc) {
    ...
    while (aux != NULL) {
        ...
        if ((total1 <= fech3) && (fech3 <= total2)) {
            cont=cont +1;
			printf("\n\nID Trabajo: %d\n", aux->idTrabajo);
            buscarnombre(nombre_opc, aux->idOpcion, opc); // Se asume que existe una funci�n llamada buscarnombre para obtener el nombre de la opci�n
            printf("Opcion: %s\n", nombre_opc);
            printf("Ubicacion: %s\n", aux->Ubicacion);
...
void buscarnombre(char *nombre, int id, struct opciones *prin) {
    while (prin != NULL) {
        if (id == prin->cod_opcion) {
            ...
            return;  // Salir de la funci�n una vez encontrado el nombre
        }
...
void contar_ventas_entre_fechas(struct Trabajos_Pendientes *E2, struct opciones *opciones, int fecha_inicio, int fecha_fin) {
    ...
    while (trabajo != NULL) {
        // Convertir la fecha del trabajo a d�as
        ...
        if (trabajo_fecha >= fecha_inicio && trabajo_fecha <= fecha_fin) {
            // Aumentar la cantidad de ventas para la opci�n actual
...
// Funci�n para listar las opciones m�s vendidas entre fechas especificadas
void listaropcvendi(struct Trabajos_Pendientes *E2, struct opciones *ini) {
...
// Funci�n para listar las 4 opciones m�s vendidas
void listar_mas_vendi(struct opciones *opciones) {
    ...
    // Mostrar las cuatro opciones m�s vendidas
    printf("Las cuatro opciones m�s vendidas son:\n");
...
void Cargar_Desde_Archivo(struct Trabajos_Pendientes **Entrada, struct Trabajos_Pendientes **Salida, const char *nombreArchivo) {
    FILE *archivo = fopen(nombreArchivo, "r");
    if (archivo == NULL) {
        printf("No se pudo abrir el archivo %s. Se comenzar� con una cola vac�a.\n", nombreArchivo);
        return;
    }
...
void agregar_cliente(struct clientes *inicio) {
    ...
    printf("Ingrese el c�digo del cliente: ");
...
void agregar_tecnico(struct tecnicos *inicio) {
    ...
    if (nuevo_tecnico == NULL) {
        printf("Error: No se pudo asignar memoria para el nuevo t�cnico.\n");
        return;
    }

    printf("Ingrese el c�digo del t�cnico: ");
    scanf("%d", &nuevo_tecnico->codTecnico);

    printf("Ingrese el nombre del t�cnico: ");
    scanf(" %[^\n]", nuevo_tecnico->nombre); // Para leer una l�nea completa, se utiliza "%[^\n]"

    printf("Ingrese el DNI del t�cnico: ");
...
void listar_trabajos_tecnico(struct Trabajos_Pendientes *E, struct Trabajos_Pendientes *E2, struct opciones *ini_opciones, struct clientes *ini_clientes) {
    ...
    if (scanf("%d", &id_tecnico) == 0) {
        printf("Error al leer el ID del t�cnico.\n");
        return;
    }
    ...
    while (aux != NULL) {
        if (aux->idTecnico == id_tecnico) {
        	printf("------------------------------------\n");
        	printf("Trabajos pendientes del t�cnico:\n");
            printf("ID Trabajo: %d\n", aux->idTrabajo);
            printf("Nombre de la opci�n: %s\n", obtener_nombre_opcion(aux->idOpcion, ini_opciones));
    ...
    while (aux2 != NULL) {
        if (aux2->idTecnico == id_tecnico) {
        	printf("Trabajos realizados del t�cnico:\n");
            printf("ID Trabajo: %d\n", aux2->idTrabajo);
...
// Funci�n para obtener el nombre del cliente dado su ID
const char* obtener_nombre_cliente(int id_cliente, struct clientes *ini_clientes) {
